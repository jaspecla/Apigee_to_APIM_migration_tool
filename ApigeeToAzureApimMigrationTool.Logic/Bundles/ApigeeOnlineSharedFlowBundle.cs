...
namespace ApigeeToAzureApimMigrationTool.Service.Bundles
{
    public class ApigeeOnlineSharedFlowBundle : IBundle
    {
        ...
        public ApigeeOnlineSharedFlowBundle(string bundleBasePath, IApigeeManagementApiService apigeeManagementApiService)
        {
            _bundleBasePath = bundleBasePath;
            _apigeeManagementApiService = apigeeManagementApiService;
        }

        public async Task LoadBundle(string sharedFlowName)
        {
            //get api metadata
            var sharedFlowMetadata = await _apigeeManagementApiService.GetSharedFlowByName(sharedFlowName);
            //get the latest revision
            int maxRevision = sharedFlowMetadata.revision.Select(x => int.Parse(x)).Max();
            //download api proxy bundle 
            _bundlePath = await _apigeeManagementApiService.DownloadSharedFlowBundle(_bundleBasePath, sharedFlowName, maxRevision);
        }
